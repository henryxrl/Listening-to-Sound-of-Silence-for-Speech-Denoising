...
def evaluate(args, save_individual_results=True, save_noise_info=True, save_stat=True, dataset_json=None, clean_audio=True):
    ...
    for i, data in enumerate(tqdm(dataloader)):
        ...
        with torch.no_grad():
            batch_output_values = torch.sigmoid(net(v=batch_frames, s=batch_audios)).detach().cpu().numpy()
            ...
            for idx in range(batch_output_values.shape[0]):
                ...
                if save_individual_results:
                    ...
                    if label != pred_label or label == 0.0 or pred_label == 0.0:
                        ...
                        # save gif
                        gif_path = os.path.join(os.path.abspath(args.outputs), 'gifs' + suffix, str(data_id)+'.gif')
                        ensure_dir(get_parent_dir(gif_path))
                        info['gif'] = os.path.join('gifs' + suffix, str(data_id)+'.gif') # need relative path!
...
def run(args, data_list, save_results=True, save_stat=False, is_ids=False):
    ...
    for data_id, data in enumerate(tqdm(data_list)):
        ...
        with torch.no_grad():
            output_value = net(frames, audio).detach().cpu().numpy()
            output_value = float(np.squeeze(sigmoid(output_value)))
            pred_label = 1.0 if output_value >= SIGMOID_THRESHOLD else 0.0
            # pred_label = (torch.sigmoid(net(frames, audio)).detach().cpu().numpy() >= SIGMOID_THRESHOLD).astype(np.float)
...
def main():
    ...
    if not args.eval:
        ...
        if not args.unknown_clean_signal:
            ...
            evaluate(args, save_individual_results=args.save_results, save_noise_info=True, save_stat=True, dataset_json=HENRIQUE_JSON, clean_audio=False)
